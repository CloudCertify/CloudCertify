...
- Amends: ADR 0008 (a practice drill is no longer only a Drill Mix, and a
...
3. **This drill writes neither Confidence nor its own membership.** The
   Confidence half moves only after the next finished full Quiz. The Outcome
   half moves through whatever
   [Do review-mode attempts feed Outcome?](https://github.com/CloudCertify/CloudCertify/issues/65)
   decides. There is no second Missed set.
